...
// Input com busca em tempo real no Firestore + seleção de item.
// ============================================================
...
export default function BuscaPecas({ onAdicionar }: BuscaPecasProps) {
  ...
  const estoqueInsuficiente =
    itemFoco !== null && qtdSelecionada > itemFoco.quantidade
  ...
  return (
    <div ref={wrapperRef} className="space-y-2">
      ...
      {/* Painel de quantidade + botão adicionar */}
      {itemFoco && (
        <div className="flex items-center gap-2 p-3 bg-orange-50 border border-orange-200 rounded-lg">
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-800 truncate">{itemFoco.nome}</p>
            <p className="text-xs text-gray-500">
              R${itemFoco.preco_venda.toFixed(2)}/{itemFoco.unidade} ·
              Disponível: {itemFoco.quantidade}
            </p>
          </div>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="w-7 h-7 rounded-md border border-gray-300 bg-white text-gray-600 hover:bg-gray-100 flex items-center justify-center text-sm font-bold transition"
            >−</button>
            <input
              type="number"
              min={1}
              max={itemFoco.quantidade}
            ...
            <button
              type="button"
              onClick={() => setQtd(q => Math.min(itemFoco.quantidade, q + 1))}
              className="w-7 h-7 rounded-md border border-gray-300 bg-white text-gray-600 hover:bg-gray-100 flex items-center justify-center text-sm font-bold transition"
            >+</button>
          </div>
          ...
          <button
            ...
            disabled={estoqueInsuficiente || qtdSelecionada <= 0}
