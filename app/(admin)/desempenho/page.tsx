'use client'
// ============================================================
// DESEMPENHO — app/(admin)/desempenho/page.tsx
// Relatório de performance por mecânico com ranking.
// ============================================================
...
import {
  ...
  Wrench, DollarSign, Clock, TrendingUp, Package,
} from 'lucide-react'
...
export default function DesempenhoPage() {
  ...
  const isUltimoMes = mesRef.getMonth() === new Date().getMonth() &&
  ...
  const mediaConc     = metricas.length > 0
    ? Math.round(metricas.reduce((s, m) => s + m.taxa_conclusao, 0) / metricas.length)
    : 0
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Métricas e ranking dos mecânicos
          </p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {[
              ...
              { label: 'Faturamento',val: `R$${totalFat.toLocaleString('pt-BR',{minimumFractionDigits:2})}`,
                icon: <DollarSign size={18} />, cor: 'text-blue-500 bg-blue-50' },
              ...
              {metricas.length >= 2 && (
                <div className="card">
                  ...
                  <div className="grid gap-3" style={{
                    gridTemplateColumns: `repeat(${Math.min(metricas.length, 3)}, 1fr)`
                  }}>
                    {metricas.slice(0, 3).map((m, i) => (
                      ...
                      >
                        ...
                        <p className="text-lg font-bold text-orange-500 mt-1">
                          R${m.faturamento_gerado.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}
                        </p>
                            ...
                            <div className="w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${
                                  m.taxa_conclusao >= 90 ? 'bg-green-500' :
                                  m.taxa_conclusao >= 70 ? 'bg-orange-400' : 'bg-red-400'
                                }`}
                                style={{ width: `${m.taxa_conclusao}%` }}
                              />
                            </div>
                            <span className={`text-xs font-semibold ${
                              m.taxa_conclusao >= 90 ? 'text-green-600' :
                              m.taxa_conclusao >= 70 ? 'text-orange-500' : 'text-red-500'
                            }`}>
                              {m.taxa_conclusao}%
                            </span>
                          </div>
